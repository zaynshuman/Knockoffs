...
DJ <- read_excel("H:\\Dataset\\DJ.xlsx")
head(DJ)
DJ$datadate <- lubridate::ymd(DJ$datadate)
DJ <- dplyr::arrange(DJ, datadate)

friday_iter = as.Date(DJ$datadate[6564])
...
friday_vec %in% DJ$datadate


updated_dates = c(1:630)
for (i in 1:630){
  updated_dates[i] <- date_near(DJ$datadate,friday_vec[i],sidepref='l')
}
...
updated_dates %in% friday_vec == friday_vec %in% DJ$datadate

price_hist = c(1:630)
for (i in 1:630){
  price_hist[i] <- DJ$prccd[DJ$datadate==updated_dates[i]]
}
...
plot(1:520,return_series_resp, type = 'l',xlab='Week',ylab = 'DJIA Weekly Return')
